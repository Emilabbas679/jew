    ...
    <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
        <div class="post d-flex flex-column-fluid" id="kt_post">
            <div id="kt_content_container" class="container-xxl">
                @include('admin.flash')
                <form enctype="multipart/form-data" id="kt_ecommerce_add_category_form" class="form d-flex flex-column flex-lg-row" data-kt-redirect="apps/ecommerce/catalog/categories.html" method="POST" action="{{route('designers.store')}}">
                    @csrf
                            ...
                            <div class="card-body pt-0">
                                @foreach(config('app.langs') as $lang)
                                    <div class="mb-10 fv-row">
                                        ...
                                        <input type="text" name="title[{{$lang['code']}}]" class="form-control mb-2" placeholder="Designer name" value="{{old('title.'.$lang['code'])}}" />
                                    </div>
                                @endforeach
                            </div>
                        </div>
