    ...
    <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
        <div class="post d-flex flex-column-fluid" id="kt_post">
            <div id="kt_content_container" class="container-xxl">
                @include('admin.flash')
                <form enctype="multipart/form-data" id="kt_ecommerce_add_category_form" class="form d-flex flex-column flex-lg-row" data-kt-redirect="apps/ecommerce/catalog/categories.html" method="POST" action="{{route('occasions.update', $occasion->id)}}">
                    @csrf
                        ...
                        <div class="card card-flush py-4">
                            ...
                            <div class="card-body pt-0">
                                ...
                                <select name="status" class="form-select mb-2" data-control="select2" data-hide-search="true" data-placeholder="Select an option" id="kt_ecommerce_add_category_status_select">
                                    <option></option>
                                    <option value="1" @if($occasion->status == 1 or $occasion->status == null) selected @endif>Published</option>
                                    <option value="2" @if($occasion->status == 2) selected @endif>Unpublished</option>
                                </select>
                            ...
                            <div class="card-body pt-0">
                                @foreach(config('app.langs') as $lang)
                                    <div class="mb-10 fv-row">
                                        ...
                                        <input type="text" name="title[{{$lang['code']}}]" class="form-control mb-2" placeholder="Category name" value="{{$occasion->getTranslation('title', $lang['code'])}}" />
                                    </div>
                                @endforeach
                            </div>
                        </div>
